...
`TimezoneManager` is an internal class -- it is not exported from `@forcecalendar/core`. Access it through a calendar instance via `calendar.timezoneManager`, or use the `Calendar` wrapper methods (`convertTimezone`, `toCalendarTimezone`, `fromCalendarTimezone`, `formatInTimezone`, `getTimezones`) shown below.

```js
// Accessed via a Calendar instance
const tz = calendar.timezoneManager;
```
...
| `convertTimezone(date, from, to)` | Convert a date between timezones |
| `getTimezoneOffset(date, timezone)` | Get UTC offset in minutes |
| `formatInTimezone(date, timezone, options)` | Format in a timezone |
| `getCommonTimezones()` | List common timezones as `Array<{ value, label, offset }>` |
...
- The `timezoneChange` event fires with `{ timezone, previousTimezone }`
...
Returns an array of common timezone descriptors -- `Array<{ value, label, offset }>` -- where `value` is the IANA identifier. (Wraps `TimezoneManager.getCommonTimezones()`.)
...
Provides static data about IANA timezones. Like `TimezoneManager`, this class is internal and is not exported from `@forcecalendar/core` -- its data is surfaced through `calendar.getTimezones()` and `calendar.timezoneManager`.
