...
These classes are internal utilities used by `EnhancedCalendar` and `EventStore` -- they are not exported from the `@forcecalendar/core` package entry point and cannot be imported directly. Their behavior is surfaced through higher-level APIs such as `calendar.eventStore.getPerformanceMetrics()` and EnhancedCalendar's performance methods. The method references below document how they work internally.
...
```js
// Internal — not importable from '@forcecalendar/core'
