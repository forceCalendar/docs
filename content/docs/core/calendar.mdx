...
Emits: `eventUpdate` with `{ event, oldEvent }`.
...
Emits: `timezoneChange` with `{ timezone, previousTimezone }`.
...
Returns a list of common timezone descriptors as `Array<{ value, label, offset }>`, where `value` is the IANA identifier.
...
Get groups of overlapping events for a specific date. Pass `timedOnly = false` to include all-day events (defaults to `true`).
...
Calculate horizontal positioning for an array of overlapping events (for visual rendering in week/day views).
...
| `eventUpdate` | `{ event, oldEvent }` | Event is updated |
| `eventRemove` | `{ event }` | Event is removed |
| `eventsSet` | `{ events }` | All events are replaced |
| `stateChange` | `{ state }` | Any state change |
| `eventStoreChange` | `{ type, event? }` | EventStore changes |
| `timezoneChange` | `{ timezone, previousTimezone }` | Timezone changes |
