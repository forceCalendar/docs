...
| `height` | `string` | `'800px'` | CSS height value |
...
| Event | `detail` | When |
|-------|----------|------|
| `calendar-navigate` | `{ action, date }` | Navigation occurs |
| `calendar-view-change` | `{ view }` | View type changes |
| `calendar-event-add` | `{ event }` | Event creation requested |
| `calendar-event-update` | `{ event }` | Event update requested |
...
<div class="force-calendar">
  <header class="fc-header">
    <div class="fc-header-left">
      <button class="fc-btn-today">Today</button>
    </div>
    <div class="fc-header-center">
      <button class="fc-nav-arrow" data-action="previous">&lt;</button>
      <h2 class="fc-title">March 2026</h2>
      <button class="fc-nav-arrow" data-action="next">&gt;</button>
    </div>
    <div class="fc-header-right">
      <button class="fc-btn fc-btn-primary" id="create-event-btn">+ New Event</button>
      <div class="fc-view-buttons">
        <button class="fc-view-button" data-view="month">Month</button>
        <button class="fc-view-button" data-view="week">Week</button>
        <button class="fc-view-button" data-view="day">Day</button>
      </div>
    </div>
  </header>
  <div class="fc-body">
...
```js
this.addListener(this.$('.fc-nav-arrow'), 'click', () => this.previous());
```
