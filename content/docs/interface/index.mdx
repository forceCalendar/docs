...
2. Creates a `StateManager`, which wraps a `Calendar` instance from `@forcecalendar/core`
...
| `StateManager` | Interface package's own state management class (`src/core/StateManager.js`) that wraps the core `Calendar` |
