...
| `getDaysInMonth` | `(date: Date) => number` | Days count |
...
| `formatTime` | `(date: Date, locale?: string, use24Hour = false) => string` | Time string |
| `parseTime` | `(timeStr: string) => { hours, minutes }` | Parsed time |
| `setTime` | `(date: Date, timeString: string) => Date` | New date |
